...
def _create_credentials():
    """Returns credentials from OAUTH_TOKEN_PATH if available, else ADC."""
    ...
    if oauth_token_path and os.path.exists(oauth_token_path):
        ...
        creds = Credentials.from_authorized_user_info(
            token_data, scopes=[_READ_ONLY_ANALYTICS_SCOPE]
        )
...
def construct_property_rn(property_value: int | str) -> str:
    ...
    if property_num is None:
        raise ValueError(
            f"Invalid property ID: {property_value}. Must be a number or 'properties/<number>'."
        )
